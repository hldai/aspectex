...
def __pretrain_bertnrdj(
        dataset, n_labels, seq_length, n_steps, batch_size, dropout, n_layers,
    ...
    bertnrdj_model = BertNRDJ(
        ...
        model_file=load_model_file, n_lstm_layers=n_layers
    )
...
if __name__ == '__main__':
    ...
    if dataset == 'se14r':
        pretrain_load_model_file = os.path.join(
            config.SE14_DIR, 'model-data/se14r-yelpr9-rest-p0_04-bert-200h.ckpt-439')
        # model_file = None
        ...
        # model_file = None
        model_file = os.path.join(config.SE15_DIR, 'model-data/se15r-yelpr9-rest-p0_04-bert-200h-l2.ckpt')
    else:
