...
class NeuRuleDoubleJointMLP:
    ...
    def __add_logits_op(self):
        ...
        with tf.variable_scope("proj-target"):
            ...
            self.W1_tar = tf.get_variable("W1", dtype=tf.float32, shape=[input_size, self.mlp_hidden_size])
            self.W2_tar = tf.get_variable("W2", dtype=tf.float32, shape=[self.mlp_hidden_size, self.n_tags])
            ...
            pred = tf.matmul(output, self.W1_tar)
            pred = tf.tanh(pred)
            pred = tf.nn.dropout(pred, self.dropout)
            pred = tf.matmul(pred, self.W2_tar) + self.b_tar
