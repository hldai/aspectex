...
class NeuRuleDoubleJointDeep:
    ...
    def __add_logits_op(self):
        if self.share_lstm:
            with tf.variable_scope("bi-lstm-l1"):
                ...
                self.lstm_output_l1 = tf.concat([output_fw, output_bw], axis=-1)
                self.lstm_output_l1 = tf.nn.dropout(self.lstm_output_l1, self.dropout)

            with tf.variable_scope("bi-lstm-l2"):
                cell_fw = tf.contrib.rnn.LSTMCell(self.hidden_size_lstm)
                cell_bw = tf.contrib.rnn.LSTMCell(self.hidden_size_lstm)
                (output_fw, output_bw), _ = tf.nn.bidirectional_dynamic_rnn(
                        cell_fw, cell_bw, self.lstm_output_l1,
                        sequence_length=self.sequence_lengths, dtype=tf.float32)
                self.lstm_output_l2 = tf.concat([output_fw, output_bw], axis=-1)
                self.lstm_output_l2 = tf.nn.dropout(self.lstm_output_l2, self.dropout)
                lstm_output1 = lstm_output2 = self.lstm_output_l2

        else:
            with tf.variable_scope("bi-lstm-1-l1"):
                cell_fw = tf.contrib.rnn.LSTMCell(self.hidden_size_lstm)
                cell_bw = tf.contrib.rnn.LSTMCell(self.hidden_size_lstm)
                (output_fw, output_bw), _ = tf.nn.bidirectional_dynamic_rnn(
                        cell_fw, cell_bw, self.word_embeddings,
                        sequence_length=self.sequence_lengths, dtype=tf.float32)
                self.lstm_output1_l1 = tf.concat([output_fw, output_bw], axis=-1)
                self.lstm_output1_l1 = tf.nn.dropout(self.lstm_output1_l1, self.dropout)

            with tf.variable_scope("bi-lstm-1-l2"):
                cell_fw = tf.contrib.rnn.LSTMCell(self.hidden_size_lstm)
                cell_bw = tf.contrib.rnn.LSTMCell(self.hidden_size_lstm)
                (output_fw, output_bw), _ = tf.nn.bidirectional_dynamic_rnn(
                        cell_fw, cell_bw, self.lstm_output1_l1,
                        sequence_length=self.sequence_lengths, dtype=tf.float32)
                self.lstm_output1_l2 = tf.concat([output_fw, output_bw], axis=-1)
                self.lstm_output1_l2 = tf.nn.dropout(self.lstm_output1_l2, self.dropout)
                lstm_output1 = self.lstm_output1_l2

            with tf.variable_scope("bi-lstm-2-l1"):
                cell_fw = tf.contrib.rnn.LSTMCell(self.hidden_size_lstm)
                cell_bw = tf.contrib.rnn.LSTMCell(self.hidden_size_lstm)
                (output_fw, output_bw), _ = tf.nn.bidirectional_dynamic_rnn(
                        cell_fw, cell_bw, self.word_embeddings,
                        sequence_length=self.sequence_lengths, dtype=tf.float32)
                self.lstm_output2_l1 = tf.concat([output_fw, output_bw], axis=-1)
                self.lstm_output2_l1 = tf.nn.dropout(self.lstm_output2_l1, self.dropout)

            with tf.variable_scope("bi-lstm-2-l2"):
                cell_fw = tf.contrib.rnn.LSTMCell(self.hidden_size_lstm)
                cell_bw = tf.contrib.rnn.LSTMCell(self.hidden_size_lstm)
                (output_fw, output_bw), _ = tf.nn.bidirectional_dynamic_rnn(
                        cell_fw, cell_bw, self.word_embeddings,
                        sequence_length=self.sequence_lengths, dtype=tf.float32)
                self.lstm_output2_l2 = tf.concat([output_fw, output_bw], axis=-1)
                self.lstm_output2_l2 = tf.nn.dropout(self.lstm_output2_l2, self.dropout)
                lstm_output2 = self.lstm_output2_l2
